(function ($) {
  ...
  */
  ...
  // Swiper Slider new
  function mySwiperSlider() {
    if ($.exists(".mySwiper")) {
      // Params
      let mainSliderSelector = ".mySwiper",
        interleaveOffset = 0.5;
  ...
  function progressSwiper() {
    ...
    const progressContents = document.querySelectorAll(
      ".autoplay-progress span"
    );

    document
      .querySelectorAll(".myProgrssSwiper")
          ...
          on: {
            autoplayTimeLeft(s, time, progress) {
              if (progressCircles[index] && progressContents[index]) {
                ...
                progressContents[index].textContent = `${Math.ceil(
                  time / 1000
                )}s`;
              }
            },
